const NavItem = (props) => {
    ...
    return (
        <div onClick={ () => props.togglePage( props.title ) } className="flex flex-col items-center icon group cursor-pointer">
            ...
            <span className={`text-label text-center mt-1 transition-all duration-300 group-hover:opacity-100 ${ isTabActive() ? "opacity-100 text-accent" : "opacity-0" }`}>{ props.title }</span>
        </div>
     );
}
