...
function ToolTipLifts({
  ...
  useEffect(() => {
    ...
    //console.log(focusFromOutside, lift.name);
    if (lift.name == focusFromOutside) {
  ...
  return (
    <>
      <div
        className={`contentLift select-none ${open ? "lineLift" : ""}`}
        onClick={toggleOpen}
      >
        <div className="flex">
          {!open && getPlantIcon(lift.type ?? "")}
          {open && (
            <p className="text-p-md font-semibold text-center text-white">
              {lift.name}
            </p>
          )}
        </div>
          ...
          <div>
            {lift.hours && (
              <div className="flex flex-col text-white ">
                <p className="text-p-xs text-center">
                  Betriebszeiten: {lift.hours}
                </p>
              </div>
            )}
            {lift.lastDescent && (
              <div className="flex flex-col text-white ">
                <p className="text-p-xs text-center">
                  Letzte Talfahrt: {lift.lastDescent}
                </p>
              </div>
            )}
            {lift.capacity && (
